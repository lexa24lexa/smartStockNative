...
def get_stock_vs_sales(
    ...
    sales_query = db.query(
     ...
     .join(models.Sale, models.SaleLine.sale_id == models.Sale.sale_id) # Join necesario para ver la fecha
...
def generate_fake_sales(db: Session = Depends(database.get_db)):
    """Generates fake sales data with different dates to test FR09"""
    import random
    ...
    db.commit()
    return {"message": f"Successfully generated {created_count} sales with different dates."}