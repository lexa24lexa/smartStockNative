...
# Get all batches, optional filter by product
@router.get("", response_model=List[schemas.BatchResponse])
def get_batches(product_id: Optional[int] = None, db: Session = Depends(database.get_db)):
...
def create_batch(batch: schemas.BatchCreate, db: Session = Depends(database.get_db)):
    ...
    if not product:
        raise HTTPException(400, "Product does not exist or is inactive")
    ...
    if existing:
        raise HTTPException(400, f"Batch code '{batch.batch_code}' already exists for this product")
...
def update_batch(batch_id: int, batch: schemas.BatchUpdate, db: Session = Depends(database.get_db)):
    ...
    if batch.batch_code or batch.product_id:
        ...
        if existing:
            raise HTTPException(400, f"Batch code '{batch_code}' already exists for this product")
...
# Delete a batch if no stock exists
...
def delete_batch(batch_id: int, db: Session = Depends(database.get_db)):
    ...
    if stock_exists:
        raise HTTPException(400, "Cannot delete batch with existing stock")
