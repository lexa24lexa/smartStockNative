...
def create_sale_fifo(sale_input: schemas.SaleCreate, db: Session = Depends(database.get_db)):
    ...
    for line in sale_input.lines:
        if line.quantity <= 0:
            db.rollback()
            raise HTTPException(status_code=400, detail=f"Quantity must be greater than 0 for batch {line.batch_id}")
    ...
    return schemas.SaleResponse(sale_id=new_sale.sale_id, store_id=new_sale.store_id, total_amount=new_sale.total_amount, date=new_sale.date, lines=sale_lines_instances)
...
def get_sale(sale_id: int, db: Session = Depends(database.get_db)):
    ...
    return schemas.SaleResponse(sale_id=sale.sale_id, store_id=sale.store_id, total_amount=sale.total_amount, date=sale.date, lines=sale.sale_lines)
...
def update_sale_fifo(sale_id: int, sale_input: schemas.SaleCreate, db: Session = Depends(database.get_db)):
    ...
    return schemas.SaleResponse(sale_id=sale.sale_id, store_id=sale.store_id, total_amount=sale.total_amount, date=sale.date, lines=sale_lines_instances)
...
def get_daily_sales_report(store_id: int, format: str = Query(..., description="Report format: 'pdf' or 'excel'"), report_date: date = None, db: Session = Depends(database.get_db)):
    ...
    total_amount = 0
    for sale in sales:
        lines = db.query(models.SaleLine, models.Batch, models.Product).join(models.Batch, models.Batch.batch_id == models.SaleLine.batch_id).join(models.Product, models.Product.product_id == models.Batch.product_id).filter(models.SaleLine.sale_id == sale.sale_id).all()
