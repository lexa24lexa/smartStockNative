    ...
    <View style={styles.progressBarBackground}>
      <View
        style={[
          ...
          { width: `${percentage}%`, backgroundColor: bgColor },
        ]}
      />
    </View>
  );
...
export default function Dashboard() {
  ...
  const [stockVsSales, setStockVsSales] = useState<CategoryStockVsSales[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [catRes, lowRes, svsRes] = await Promise.all([
        fetch("http://127.0.0.1:8000/analytics/stock-by-category"),
        fetch(`http://127.0.0.1:8000/analytics/low-stock?store_id=${storeId}`),
        fetch("http://127.0.0.1:8000/analytics/stock-vs-sales"),
      ]);

      const catData: CategoryStock[] = await catRes.json();
      const lowData: ReplenishmentItem[] = await lowRes.json();
      const svsData: CategoryStockVsSales[] = await svsRes.json();

      setCategories(catData);
      setReplenishments(lowData);
      setStockVsSales(svsData);
  ...
  const stats = useMemo(() => {
    ...
    const outOfStock = categories.filter(
      (c) => c.total_stock === 0
    ).length;

    return {
      availability: total
        ? Math.round(((total - outOfStock) / total) * 100)
        : 0,
      outOfStock,
  ...
  const lowStockItems = useMemo(
    () => replenishments.filter((i) => i.current_stock < 50),
    [replenishments]
  );
  ...
  const labels = stockVsSales.map((c) => c.category);
  const stockValues = stockVsSales.map((c) => c.stock);
  const salesValues = stockVsSales.map((c) => c.sales);
  ...
  const xScale = (i: number) =>
    labels.length === 1
      ...
      : padding + (i / (labels.length - 1)) * chartInnerWidth;

  const yScale = (v: number) =>
    padding + (1 - v / maxValue) * (chartHeight - padding * 2);
  ...
  const yAxisValues = [0, 0.25, 0.5, 0.75, 1].map((f) =>
    ...
    <Layout onRefresh={fetchData}>
      ...
      >
        ...
        <Text style={Font.subtitle}>
          {loading ? "Loading…" : getUpdatedText()}
        </Text>
        ...
        <View style={styles.section}>
          ...
          {stockVsSales.length === 0 ? (
            <Text style={Font.subtitle}>
              No stock or sales data available.
            </Text>
          ) : (
            <>
              ...
              <View style={styles.legend}>
                <View style={styles.legendItem}>
                  ...
                  />
                  <Text style={Font.subtitle}>Stock</Text>
                </View>
                <View style={styles.legendItem}>
                  <View
                    style={[
                  ...
                  />
                  <Text style={Font.subtitle}>Sales</Text>
                </View>
              </View>
            </>
          )}
          ...
          {lowStockItems.length === 0 ? (
            ...
            lowStockItems.map((item) => {
              ...
              return (
                ...
                >
                  <Text style={[Font.label, { marginBottom: 4 }]}>
                    {item.product_name}
                  </Text>
                  <Text
                    style={{
